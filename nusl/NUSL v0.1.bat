...
if not exist temp\drive.txt goto:DRIVECHANGE2
if not exist temp\driveu.txt goto:DRIVEUCHANGE2
...
echo.
echo    Willkommen beim NUSL! Heute ist der %DATE%!
echo.
::------------------------Spruch Depository------------------------------------------------
if %DATE% == 28.12.2011 echo Und? Wie ist es so in der Nachweihnachtszeit?
if %DATE% == 29.12.2011 echo Was ist bloá die Wurzel aus 5?
if %DATE% == 30.12.2011 echo Morgen ist Silvester! Schon was vor?
if %DATE% == 31.12.2011 echo Lass es krachen! Silvester!
if %DATE% == 01.01.2012 echo Frohes neues Jahr!
if %DATE% == 02.01.2012 echo Ich bin immer noch in Partylaune!
if %DATE% == 03.01.2012 echo Wenn du in die Optionen gehst und S+ eingibst soll angeblich etwas geschehen...
if %DATE% == 04.01.2012 echo Es fehlt ein Auto Updater in NUSL...
if %DATE% == 05.01.2012 echo Morgen ist mein Geburtstag! Schenkst du mir was?
if %DATE% == 06.01.2012 echo Jahu heute ist mein Geburtstag! Wie schön!
if %DATE% == 07.01.2012 echo Netter Samstag!
if %DATE% == 08.01.2012 echo 8. Januar 2012! Die Welt geht in 11 Monaten unter...
if %DATE% == 09.01.2012 echo Das Programm ist von den Sprchen zugemllt...
if %DATE% == 10.01.2012 echo Nein diese Sprche gibt es nicht zum Download!
if %DATE% == 11.01.2012 echo if not exist %DRIVE% mkdir %DRIVE%...
echo.
...
if /i "%MENU%" EQU "U" goto:updatenusl
...
echo   =============
echo   SD = SD Speicherort „ndern
echo   USB = USB Speicherort „ndern
...
if /i "%CREDIT3%" EQU "SD" goto:DRIVECHANGE
if /i "%CREDIT3%" EQU "USB" goto:DRIVEUCHANGE
...
echo.
echo.
echo                    Danke, dass du NUSL heruntergeladen hast!
echo.
echo		Vorher mssen wir aber noch ein paar Einstellungen festlegen!
echo.
echo	   W„hle bitte aus, wo Sachen gespeichert werden sollen, die auf die SD mssen!
echo.
...
echo.
echo		N = Nein beides stimmt nicht!
echo.
...
echo					   %DATE%
echo.
echo                            Aktuelle Version ist %CurrentVersion%
echo.
...
if exist NUSLautoupdate.bat del NUSLautoupdate.bat
start %NUSLmin%/wait support\wget -N "http://code.google.com/p/nusdl/downloads/list?can=3&q=&colspec=Filename+Summary+Uploaded+ReleaseDate+Size+DownloadCount"
...
if %currentversion% GTR %newversion:~0,5% (echo                  Diese Version ist höher als die letzte Version.) & (echo.) & (echo                           Du hast jetzt eine coole Beta Version!) & (@ping 127.0.0.1 -n 4 -w 1000> nul) & (goto:a)

::before %updatename% is called %newversion% can be more than 5 chars (ie. 4.6.0.1)
if %currentversion% EQU %newversion% (echo                              Diese Version ist aktuell!) & (@ping 127.0.0.1 -n 4 -w 1000> nul) & (goto:a)
...
if %NEWDBUPDATEVERSION% EQU %DBUPDATEVERSION% (echo                              Diese Version ist aktuell!) & (@ping 127.0.0.1 -n 4 -w 1000> nul) & (goto:a)

:getchangelog
start %NUSLmin%/wait support\wget "http://nusdl.googlecode.com/files/%UPDATENAME%-%newversion%.txt"
if not exist %UPDATENAME%-%newversion%.txt goto:updatefail

ren %UPDATENAME%-%newversion%.txt %UPDATENAME%.bat
call %UPDATENAME%.bat else goto:updatefail

set newversion=%newversion:~0,5%
if %currentversion% GEQ %newversion% (goto:a) else (exit)

:updatefail
echo   Die Suche nach Updates ist fehlgeschlagen!
echo   Überprüfe deine Internet Verbindung und deine Firewall
@ping 127.0.0.1 -n 5 -w 1000> nul
